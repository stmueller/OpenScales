...
| Field | Type | Description |
|-------|------|-------------|
| `items` | array or object | Item IDs to include. Accepts two formats: (1) **Array of strings** — all items are forward-coded (coding = 1). Example: `["q1", "q2", "q3"]`. (2) **Object mapping item IDs to coding values** — `1` (forward), `-1` (reverse), `0` (exclude). Example: `{"q1": 1, "q2": -1, "q3": 1}`. The array format is preferred when all items are forward-coded (the common case). |
...
| `item_coding` | object | **Deprecated** — use the object format of `items` instead. Legacy field for backward compatibility: per-item coding `1` (forward), `-1` (reverse), `0` (exclude). Runners should check `items` first, falling back to `item_coding` if present. |
